...
> **NON-NEGOTIABLE MINIMUMS — SELF-CHECK BEFORE PUBLISHING (added 2026-07-30 after repeated misses):**
> These have each been dropped from a real published report at least once. Before any CMA (interactive HTML, buyer mode, listing mode, past-client mode, quick pricing question — no exceptions) is published or sent, confirm all four out loud:
> 1. **Concrete dollar price points, always.** Never end a report on qualitative-only guidance ("price to compete," "move decisively"). Every report gets at least one explicit $ number the client can act on — a list price, an offer range, or (when a genuine unresolved fact like unverified square footage changes the number) multiple labeled dollar scenarios, each with a real range and a stated confidence level. "It depends" is not a deliverable; "$X to $Y if A, $Z to $W if B" is.
> 2. **Prior sale history in the headline stats, not buried.** If the subject property has ANY prior recorded sale, its last sold price AND date go in the top Bottom-Line stats row (Section 1/2), not only in a price-history table further down the page. A client should never have to scroll to learn what the house last sold for.
> 3. **Minimum three charts, always.** A report with only one chart is incomplete. At minimum: (a) a comp price comparison chart, (b) a days-on-market or list-to-sale chart, (c) a third relevant to the property's specific story — a price-history-over-time line if there's sale history, a $/sqft comparison, a cap-rate table, or a rate-sensitivity chart. If the data genuinely doesn't support a third chart, say so explicitly to the user rather than silently shipping two.
> 4. **Every material red flag gets a number attached, not just a warning.** If something is unverified (square footage, permits, condition claims), don't just flag it, show what the price recommendation looks like in each scenario (see #1). A flag without a number forces the client to do the math themselves.
...
Write the output HTML into the Online Content clone at `C:/Users/Graeham Watts/Documents/Skills LLMS/Claude/Online Content/cma/CMA_[address].html`, then:

```bash
cd "C:/Users/Graeham Watts/Documents/Skills LLMS/Claude/Online Content"
git add "cma/CMA_[address].html"
git -c user.name="Graeham Watts" -c user.email="graehamwatts@gmail.com" commit -m "CMA: [address]"
