...
**"Confirm the interior condition" / "I haven't personally seen inside" hedging** — banned in listing-mode (pre-listing) CMAs. This report goes to a seller who is imminently listing, prepared by their own agent. Both of them already know what the house looks like inside; a caveat telling them to "confirm current condition before finalizing the list price" or noting a dollar swing "if a walkthrough confirms X" reads as absurd, not careful. Real incident, 2026-08-17 (16 Clarence Court): the report carried this hedge in five separate places (the subject details table, the market-story paragraph, the comp table's subject row, a dedicated Pricing Strategy paragraph, and a full Special Considerations bullet), and Graeham flagged it as "so stupid because we both know what the house is already like." Price off the most recently known condition as settled fact, stated once, in the subject details table. No walkthrough disclaimer, no dollar-swing-if-unconfirmed math, no "have not had eyes on the interior" language anywhere in the report.

This does NOT apply to past-client "home value update" mode, where the homeowner receiving the report may genuinely not have shared recent renovation details with Graeham — there, a brief, one-time "let's confirm this still reflects the home" line in the CTA is fine. It also doesn't apply when Graeham explicitly tells Claude the condition is unverified for a specific reason (absentee owner he's never spoken to, a listing he hasn't toured) — then say so once, plainly, without the swing-math treatment, and don't repeat it in multiple sections.
