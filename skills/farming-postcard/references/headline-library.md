...
**MAILED 2026-08-01** (confirmed by Graeham). Three cards in a row have now shipped on the call/text channel (Prop 19 → Buyer-tagged → Equity), which breaks the CTA-destination-cooldown spirit even though each used a distinct SMS keyword — **the landing-page fast-follow is now overdue.**

**Design note (2026-07-27):** Graeham flagged the mailed 08/01 card visually drifted from the clean 2025 house style — a cream back panel (2025 cards run copy directly on white), a brighter/glossier gold instead of the flat brass border-gold, and an unbalanced front headline with the arrow pointing at empty space. Punch list sent to Peter: drop the cream panel, match the flat brass gold exactly, widen/raise the headline, point the arrow at the QR, tighten back copy to 2 lines. Apply this same style check to every future card before it goes to print — compare against 03/01/25, 03/15/25, or 04/15/25 as the reference silhouette.
