...
## AVATAR CHARACTER REFERENCE & GENERATION PIPELINE (adopted 2026-08-31; verified against the current Higgsfield/HeyGen/Kling/ElevenLabs stack)

Poor avatar quality traces to poor REFERENCE INPUTS, not the render tools. This section is the standard; Peter/editors follow it before any render, and every generated-Graeham piece inherits it.

**The reference-set standard (Soul ID / character training):**
- ~20 images, mixed poses AND styles, MUST include tight close-ups (the face detail source), clean simple backgrounds, soft even light. CURRENT LIKENESS ONLY — nothing older than ~12 months. Keep skin marks/moles/pores; imperfections carry identity — never beautify references.
- Gaps in the set are filled SYNTHETICALLY: portrait prompt formula = "45-degree angle portrait [/ strict left profile / chin raised], neutral expression, clean white studio background, soft studio lighting, ultra realistic skin texture, sharp detailed eyes, 85mm lens, shallow depth of field, photorealistic, maintain character consistency with hair, eyes, nose, lips, skin texture including any spots, moles, pores, and natural facial features" — generated with Nano Banana Pro/2 in Higgsfield against a real close-up, then added to the training set.
- Fast path when no set exists: ONE good selfie → Nano Banana character sheet (multi-angle turnaround). Quality path: full Higgsfield Soul ID trained on the 20-image set.
- **Graeham's canonical reference assets** (inventoried 2026-08-31): `C:\Users\Graeham Watts\Pictures\Pictures and avatar shots of me\` — the Oct-Dec 2025 AR-series pro session (frontal + profile + full body, navy shirt, clean white walls, 9504x6336) with `2025-12-09 12 57 05.jpg` as the single best face close-up; supplement expression range from 2025 files in `Dating Picture's of me` (e.g. IMG_6115.JPG) and the `HEIC` folder. Do NOT use `Ai Headshots` (synthetic, idealized) as identity ground truth, and nothing pre-2020. Box mirror: "Headshot for Higgsfield Soul" folder (349915343540, 64 files). Full media index: `WESLEY-MEDIA-MASTER-MANIFEST.md` in the same Pictures folder.

**The generation pipeline (short-form talking clips):** approved scene image first (Nano Banana 2 + character sheet, 9:16) → image-to-video in Kling 3.0 with the SPOKEN LINE inside the prompt plus delivery direction and gesture beats ("fixed camera, says with sharp conviction: '...'"), clip duration matched to the line → voice pass: ElevenLabs Professional Voice Clone (the paid PVC, 30+ min training audio — Graeham's 63 MP4 takes in the canonical folder are the training source) → lip-sync pass in Higgsfield. New camera angles come from EDITING the approved frame ("change angle to slightly elevated side shot, do not change pose/color") — never regenerating from scratch. Long-form: HeyGen Avatar IV from one screenshot + the PVC audio. Green-screen trick for composites: Nano Banana "background to pure green," HeyGen animate, CapCut chroma removal.

**Rules that make it work:** (1) generate many, pick one — pros run dozens of generations per keeper; budget credits accordingly and never ship the first take; (2) static/seated shots only for lip-sync (motion + lip-sync warps faces — editor-confirmed); (3) the LLM writes generation prompts from a rules file + the character sheet — humans describe the shot in plain language; (4) prompt grammar for b-roll: CAMERA + SUBJECT + ACTION (+ ENVIRONMENT/LIGHTING/TEXTURE optional), consistent with `cinematic-hooks`.
