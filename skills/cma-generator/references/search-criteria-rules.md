...
### MLS date filters are NOT trustworthy until proven otherwise (hard rule, added 2026-08-07 after a real incident)
A CMA for 456 62nd St, Oakland was sent to a client with every comp dated 2021-2023, presented as "how this pocket is trading right now," even though a COE/Sale Date filter had been set for "last 18 months." The filter silently did not apply. Mid-search, records outside the intended window (a 2014 sale, a 2004 sale) appeared in results that should have excluded them, this was visible evidence the filter had failed, and it was not acted on: the session proceeded to hand-pick "recent-looking" comps by eye instead of stopping to fix the filter or verify dates individually. The client caught it, not the process. This must never happen again:

- **Never trust a date-range field in MLS Matrix (or any MLS search tool) just because it was typed in.** These fields are known to silently fail to apply in this exact search interface. Treat every date filter as unverified until proven otherwise.
- **After setting any date filter, spot-check it before using a single result.** Open at least 2-3 of the returned records and confirm their actual close/sale dates fall inside the intended window. If even one falls outside it, the filter did not apply, stop and rebuild the search (sort by distance or price instead, then verify dates by hand) rather than continuing to trust the filter.
- **Every comp that goes into a report's pricing math must have its actual closing date verified by opening the individual record, not inferred from a search filter, a grid column, or "days on market" (DOM measures time-to-sale, not how long ago the sale happened, and does not indicate recency on its own).**
- **Never blend comps from meaningfully different rate/market cycles into one "current market" narrative.** 2021 (near-zero mortgage rates, historic frenzy) and 2023-2026 (6-7% rates) are not the same market. If the closest available comps span eras like this, either restrict the pricing math to the recent-era comps only, or split them into clearly separate, separately-labeled tables (e.g. "Verified Current Comps, Last 18 Months" vs. "Older-Cycle Comps, Historical Context Only, Not Used In Pricing"). Never let an older-era comp's list-to-sale percentage or DOM be cited as evidence of "how the market is behaving right now."
- **If genuinely recent comps are thin within the target radius, say so explicitly and use fewer, verified comps rather than padding the set with older sales presented as current.** A three-comp report built on individually-verified 18-month-old-or-newer sales is more honest and more defensible than a nine-comp report where a third of the comps are silently 3-5 years stale.
- **Before delivering any CMA, do a final date sanity pass**: read the sold-comp table and confirm every date shown is inside the window claimed in the report's own narrative (e.g. if the report says "sales since 2021" or "recent comps," every row must actually match that claim). This is a distinct check from the general Quality Control Verification in `references/output-formats.md`, run it specifically because this exact failure mode has already shipped once.
