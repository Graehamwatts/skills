...
for i, r in enumerate(records, start=2):
    ...
    ws.cell(row=i, column=7, value=r.get('beds') or None)
    ws.cell(row=i, column=8, value=r.get('baths') or None)
    sqft = r.get('sqft')
    ws.cell(row=i, column=9, value=sqft.replace(',', '') if sqft else None)
    ws.cell(row=i, column=10, value=r.get('lot_size') or None)
