...
- **Market trend chart source check (hard rule, updated 2026-08-13 after the third repeated failure — see `references/charts.md` #6): confirm an actual MLS Stats screenshot exists in this session's tool calls for the market trend chart, and that the report embeds that screenshot (or a Chart.js recreation checked against it).** A line connecting a handful of individually-verified comps, or a chart with no corresponding screenshot tool call, is not a real trend chart, it's a guess dressed up as one. If no screenshot was taken, the report is not ready to ship — go pull it from MLS Stats (Search → Stats → Customize: Time Frame / Statistic / Smooth Line / Group By Month → screenshot the Chart tab) before delivering. State explicitly to the user, in your own response, that a real MLS Stats screenshot was taken.
