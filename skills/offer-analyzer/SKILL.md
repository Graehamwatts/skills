...
2. **Quiet strength-ordering, no verdicts.** DO order the side-by-side comparison by overall strength (strongest offer first — even if the edge is slight). But do it quietly: no "strongest"/"best"/"winner" labels, no scores, no explanation of why one is on top, and no recommendation of which to take. The order itself is the only signal. Verdict language and recommendations only when the user explicitly asks.
