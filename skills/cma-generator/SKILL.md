...
7a. **Target Price Reality Check** — only if the client has stated a specific target price the computed range doesn't support. See "Target Price Reality Check" in `references/pricing-behavior-analysis.md` for the required structure (real proof-of-price comp, reverse-engineered sqft/condition gap, market-velocity delta). Placed after the value range since it references it. Never argue with the client's number directly; show what it would take.
