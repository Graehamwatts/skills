...
> **PLACEMENT + SYNC RULE for every NEW skill (added 2026-06-11).** There is exactly ONE skills folder: `C:\Users\Graeham Watts\Documents\Skills LLMS\Claude\Skills\skills\`. Create every new skill folder THERE (never in `~/.claude/skills` directly — that path is a junction into the same folder — and never in the Cowork cache). After creating or editing any skill:
