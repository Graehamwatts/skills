...
**News story landing pages (added 2026-07-03).** The "What's happening near you" strip runs THREE items per send (one local development, one broader market/policy item, one "worth knowing" interest piece like the tech/AI angle). Every item links to its own story page on the online-content site — never a raw external link, never a dead end. URL convention: `news/YYYY-MM-story-slug.html` (hosted at `graehamwatts.github.io/online-content/news/...`). Page template (see the three 2026-07 pages as reference): gold top bar + eyebrow "The EPA Report · Local News" + Anton headline + 3 short humanized paragraphs in Graeham's voice + a gold-bordered "What this means for you" box with the two-axis reframe (owner / investor / buyer lines) + a source citation link + ONE home-value CTA with phone + DRE footer + back-link to the month's newsletter. Pages are generated by this skill from the intelligence-layer story log each month, run through the humanizer and brand validator, and pushed with the newsletter. In the email, story links are Switchy-wrapped like everything else, so per-story click data feeds topic selection. **GHL's role:** GHL does NOT host these story pages — it hosts the conversion endpoints only (the home-value form, plus referral and schedule forms). Story pages live on online-content where they can be auto-generated; their CTAs route INTO GHL.
